function [fint,Kt,mpData] = detMPsTest(uvw,mpData)
...
for mp=1:nmp
    ...
    mpData(mp).uvw=uvw(reshape(mpData(mp).ed,1,nn*nD));
end
...
for mp=1:nmp                                                             % material point loop
    ...
    ddF(fPos) = G*mpData(mp).uvw;                                           % spatial gradient (start of lstp) of displacements
    ...
    elseif mpData(mp).cmType == 2
        ...
        Ksig  = 0;                                                          % added to eliminate warnings
        epsE = 0;
    end
    %----------------------------------------------------------------------
    ...
    mpData(mp).kptemp = mpData(mp).vp*det(dF)*(G.'*A(aPos,aPos)*G);                    % material point stiffness contribution
    mpData(mp).fptemp = mpData(mp).vp*det(dF)*(G.'*sig(sPos));                         % internal force contribution
...
for mp=1:nmp
    dNx = mpData(mp).dSvp;                                                  % basis function derivatives (start of lstp)
    nn  = size(dNx,2);                                                      % no. dimensions & no. nodes
    ed=reshape(mpData(mp).ed,1,nn*nD);
    kp=mpData(mp).kptemp;
    fp=mpData(mp).fptemp;
