function [Result,fint,Kt,mpData] = detMPsTestNorm(uvw,mpData)
...
for mp=1:nmp                                                                % material point loop
    ...
    TestV = mpData(mp).vp*det(dF);
    kp = TestV*(G.'*A(aPos,aPos)*G);                        % material point stiffness contribution
    fp = TestV*(G.'*sig(sPos));                             % internal force contribution
